...
Transponder Codes:<UL>"}
		...
		for(var/key in codes)
			t += "<LI>[key] ... [codes[key]]"
			t += "	<A href='byond://byond://?src=[REF(src)];edit=1;code=[key]'>Edit</A>"
			t += "	<A href='byond://byond://?src=[REF(src)];delete=1;code=[key]'>Delete</A><BR>"
		t += "	<A href='byond://byond://?src=[REF(src)];add=1;'>Add New</A><BR>"
		t+= "<UL></TT>"
