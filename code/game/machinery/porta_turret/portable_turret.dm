...
/obj/machinery/porta_turret/proc/InterceptClickOn(mob/living/requester, params, atom/A)
	if(!manual_control)
		return FALSE
	if(!can_interact(requester))
		remove_control()
		return FALSE
	log_combat(requester,A,"fired with manual turret control at")
	target(A)
	return TRUE
