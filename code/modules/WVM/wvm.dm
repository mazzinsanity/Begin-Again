		...
		if(STATE_IDLE)
			...
			if(content.len == 0)
				...
				for(var/obj/item/Itm in content)
					...
					dat += "<a href='byond://byond://?src=\ref[src];vend=[item_name];current_price=[price]'>[Itm.name] | [price] caps</a> "
					dat += "<a href='byond://byond://?src=\ref[src];examine=[item_name];current_price=[price]'>Examine</a><br> "
