...
/mob/living/simple_animal/bot/mulebot/get_controls(mob/user)
	...
	if(locked && !ai && !IsAdminGhost(user))
		dat += "&nbsp;<br /><div class='notice'>Controls are locked</div><A href='byond://byond://?src=[REF(src)];op=unlock'>Unlock Controls</A>"
	else
		dat += "&nbsp;<br /><div class='notice'>Controls are unlocked</div><A href='byond://byond://?src=[REF(src)];op=lock'>Lock Controls</A><BR><BR>"
		...
		dat += "<A href='byond://byond://?src=[REF(src)];op=report'>Toggle Delivery Reporting</A> ([report_delivery ? "On" : "Off"])<BR>"
		if(load)
