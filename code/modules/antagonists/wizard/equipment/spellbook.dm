...
/obj/item/spellbook/attack_self(mob/user)
	...
	for(var/category in categories)
		cat_dat[category] = "<hr>"
		dat += "<li><a [tab==category?"class=selected":""] href='byond://byond://?src=[REF(src)];page=[category]'>[category]</a></li>"
	...
	for(var/i=1,i<=entries.len,i++)
		...
		if(E.CanBuy(user,src))
			spell_info+= "<a href='byond://byond://?src=[REF(src)];buy=[i]'>[E.buy_word]</A><br>"
		else
			spell_info+= "<span>Can't [E.buy_word]</span><br>"
		if(E.CanRefund(user,src))
			spell_info+= "<a href='byond://byond://?src=[REF(src)];refund=[i]'>Refund</A><br>"
		spell_info += "<hr>"
