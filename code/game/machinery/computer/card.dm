	...
	if (authenticated && inserted_modify_id)
		...
		if(istype(src, /obj/machinery/computer/card/centcom)) // REE
			...
			for(var/A in get_all_ncr_access())
				if(A in inserted_modify_id.access)
					accesses += "<a href='byond://?src=[REF(src)];choice=access;access_target=[A];allowed=0'><font color=\"6bc473\">[replacetext(get_ncr_access_desc(A), " ", "&nbsp")]</font></a> "
				else
					accesses += "<a href='byond://?src=[REF(src)];choice=access;access_target=[A];allowed=1'>[replacetext(get_ncr_access_desc(A), " ", "&nbsp")]</a> "
		else if(istype(src, /obj/machinery/computer/card/legion))
			accesses += "<h5>Caesar's Legion:</h5>"
			for(var/A in get_all_legion_access())
				if(A in inserted_modify_id.access)
					accesses += "<a href='byond://?src=[REF(src)];choice=access;access_target=[A];allowed=0'><font color=\"6bc473\">[replacetext(get_legion_access_desc(A), " ", "&nbsp")]</font></a> "
				else
					accesses += "<a href='byond://?src=[REF(src)];choice=access;access_target=[A];allowed=1'>[replacetext(get_legion_access_desc(A), " ", "&nbsp")]</a> "
		else if(istype(src, /obj/machinery/computer/card/enclave))
			accesses += "<h5>Enclave:</h5>"
			for(var/A in get_all_enclave_access())
				if(A in inserted_modify_id.access)
					accesses += "<a href='byond://?src=[REF(src)];choice=access;access_target=[A];allowed=0'><font color=\"6bc473\">[replacetext(get_enclave_access_desc(A), " ", "&nbsp")]</font></a> "
				else
					accesses += "<a href='byond://?src=[REF(src)];choice=access;access_target=[A];allowed=1'>[replacetext(get_enclave_access_desc(A), " ", "&nbsp")]</a> "
		else if(istype(src, /obj/machinery/computer/card/bos))
			accesses += "<h5>Brotherhood of Steel:</h5>"
			for(var/A in get_all_bos_access())
				if(A in inserted_modify_id.access)
					accesses += "<a href='byond://?src=[REF(src)];choice=access;access_target=[A];allowed=0'><font color=\"6bc473\">[replacetext(get_bos_access_desc(A), " ", "&nbsp")]</font></a> "
				else
					accesses += "<a href='byond://?src=[REF(src)];choice=access;access_target=[A];allowed=1'>[replacetext(get_bos_access_desc(A), " ", "&nbsp")]</a> "
		else
